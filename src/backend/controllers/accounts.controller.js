...
function signup(req, res) {
  ...
  // const user = accountsService.checkByPhone(phone);
  // if (user) {
  //   return res.redirect("/accounts/signup?error=1"); // Account existed
  // }
  // if (password != passwordAgain) {
  //   return res.redirect("/accounts/signup?error=2"); // passwords aren't the same
  // }
  // try {
  //   accountsService.addAccount(phone, password);
  //   res.json({
  //     success: true,
  //     timestamp: new DatshowSignupPagee().toISOString(),
  //   });
  // } catch (err) {
  //   resres.status(500).json({ success: false, error: err.message });
  // }
